...
public class RangedScript : MonoBehaviour
{
    ...
    private float nextLaserDelay = 1.0F;
    private float nextClearLaser = 0.0F;
    private float nextClearLaserDelay = 0.3F;
    private bool needLaserClear = false;
    #endregion
    ...
    private EnemyUnitScript lastUnitScript;
    private EnemyBuildingScript lastBuildingScript;
    ...
    private void FixedUpdate()
    {
        ...
        else if (isIdle || isAttackingBuilding || isAttackingUnit)
        {
            ...
            _animator.SetBool(AnimStateList.IS_MOVING_DOWN_RANGED, false);
            _animator.SetBool(AnimStateList.IS_MOVING_UP_RANGED, false);
            _animator.SetBool(AnimStateList.IS_MOVING_LEFT_RANGED, false);
            _animator.SetBool(AnimStateList.IS_MOVING_RIGHT_RANGED, false);
            _animator.SetBool(AnimStateList.IS_IDLE_RANGED, true);
        }
    ...
    private void IdleSightAttack()
    {
        ...
        if (objectInSightTeamTag.Equals(TagList.ENEMY_UNIT))
        {
            MoveAndAttackEnemyUnit(objectInSight, objectInSight.transform.position);
        }
        else if (objectInSightTeamTag.Equals(TagList.ENEMY_BUILDING))
        {
    ...
    public void MoveAndAttackEnemyUnit(GameObject enemyUnit, Vector2 location)
    {
        ...
        lastUnitScript = enemyUnit.transform.GetChild(0).gameObject.GetComponent<EnemyUnitScript>();
        _currentWaypoint = 0;
        _seeker.StartPath(_transform.position, new Vector3(movePos.x, movePos.y, 0), OnPathComplete);
        lastGeneratePathTime = Time.time;
    }
    ...
    public void MoveAndAttackEnemyBuilding(GameObject enemyBuilding, Vector2 location)
    {
        ...
        lastBuildingScript = enemyBuilding.transform.GetChild(0).gameObject.GetComponent<EnemyBuildingScript>();
        _currentWaypoint = 0;
        _seeker.StartPath(_transform.position, new Vector3(movePos.x, movePos.y, 0), OnPathComplete);
        lastGeneratePathTime = Time.time;
    }
    ...
    private void CheckAttackBuilding()
    {
        ...
        else if (isAttackingBuilding && lastBuilding)
        {
            FireLaser(lastBuilding);
        }
    ...
    private void CheckAttackUnit()
    {
        ...
        else if (isAttackingUnit && lastEnemyUnit)
        {
            FireLaser(lastEnemyUnit);
        }
    ...
    {
        return Vector2.Distance(workerPosition, otherPosition) <= UnitRangesList.RANGED;
    }
    ...
    private void checkAnimation(Vector2 direction)
    {
        if (Mathf.Abs(direction.x) > Mathf.Abs(direction.y))
        {
            if (direction.x > 0)
            {
                ...
                _animator.SetBool(AnimStateList.IS_MOVING_UP_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_LEFT_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_RIGHT_RANGED, true);
            }
            else
            {
                _animator.SetBool(AnimStateList.IS_MOVING_DOWN_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_UP_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_LEFT_RANGED, true);
                _animator.SetBool(AnimStateList.IS_MOVING_RIGHT_RANGED, false);
            }
            _animator.SetBool(AnimStateList.IS_IDLE_RANGED, false);
        }
        else
        {
            if (direction.y > 0)
            {
                _animator.SetBool(AnimStateList.IS_MOVING_DOWN_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_UP_RANGED, true);
                _animator.SetBool(AnimStateList.IS_MOVING_LEFT_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_RIGHT_RANGED, false);
            } else
            {
                _animator.SetBool(AnimStateList.IS_MOVING_DOWN_RANGED, true);
                _animator.SetBool(AnimStateList.IS_MOVING_UP_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_LEFT_RANGED, false);
                _animator.SetBool(AnimStateList.IS_MOVING_RIGHT_RANGED, false);
            }
    ...
    {
        if (nextLaser <= Time.time && target)
        {
            nextLaser += nextLaserDelay;
            ClearLaser();
            DrawLaser(target.transform);
            nextClearLaser = Time.time + nextClearLaserDelay;
            needLaserClear = true;
        }
    }

    private void CheckClearLaser()
    {
        if (isMoving)
        {
            ClearLaser();
        }
        else if (needLaserClear && nextClearLaser <= Time.time)
        {
            ClearLaser();
        }
    }
    ...
    private void DrawLaser(Transform target)
    {
        float offsetY = 0.4F;
        float offsetX = 0.1F;
        float offsetX_N = (_transform.position - target.position).normalized.x * offsetX;

        Vector3 laserStartPos = new Vector3(_transform.position.x - offsetX_N, _transform.position.y + offsetY);
        _laserRenderer.positionCount = 2;

        _laserRenderer.startWidth = 0.08f;
        _laserRenderer.endWidth = 0.04f;

        _laserRenderer.SetPosition(0, laserStartPos);
        _laserRenderer.SetPosition(1, target.position);
    }
