...
public class UnitScript : MonoBehaviour
{
    ...
    private Achievement_Manager _achievement_Manager;
    ...
    private WorkerScript _workerScript;

    // Start is called before the first frame update
    void Start()
    {
        _sortingLayerOrder_Manager = SortingLayerOrder_Manager._Instance;
        _achievement_Manager = Achievement_Manager._Instance;
    ...
    private void DecipherTypeOfUnit()
    {
        ...
        {
            currentHealth = HealthList.WORKER_HEALTH;
            maxHealth = HealthList.WORKER_HEALTH;
            _workerScript = GetComponent<WorkerScript>();
        }
    }
    ...
    private void CheckHealth()
    {
        if (currentHealth <= 0)
        {
            _achievement_Manager.SetGoodUnitDied(true);
            Destroyed();
        }
    }
    ...
    public int GetMetalAmount()
    {
        if (_workerScript)
        {
            return _workerScript.GetMetalAmount();
        }
        return 0;
    }
    public int GetElectricityAmount()
    {
        if (_workerScript)
        {
            return _workerScript.GetElectricityAmount();
        }
        return 0;
    }
    public int GetPlatinumAmount()
    {
        if (_workerScript)
        {
            return _workerScript.GetPlatinumAmount();
        }
        return 0;
    }
